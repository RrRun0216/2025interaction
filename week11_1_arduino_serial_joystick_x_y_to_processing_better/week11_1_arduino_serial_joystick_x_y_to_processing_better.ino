...
void loop() {
  ...
  int x = analogRead(A2);   // 把搖桿的 X 接到 A2
  int y = analogRead(A3);   // 把搖桿的 Y 接到 A3
  if(count<20) { //想收20筆資料
    totalX += x; //加總,要算平均值
    totalY += y; //加總,要算平均值
    count++; //又讀到一筆
  ...
  else x = (x-x0)/4.4 + 128; //有大的數值,就減掉中間值,在除4,在加128
  if( abs(y-y0) < 25 ) x = 128; //數值變化太小,直接放中間值128
  else x = (y-y0)/4.4 + 128; //有大的數值,就減掉中間值,在除4,在加128
  Serial.write(x);  
  Serial.write(y);
  //if (x > 900) tone(8, 784, 100); 
  //if (x > 700) tone(8, 523, 100); 
  //if (x > 500) tone(8, 659, 100); 
  //if (y > 900) tone(8, 500, 100); //的亂數數字
}